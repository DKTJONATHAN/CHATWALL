#!/usr/bin/env python3
"""CHATWALL debate agent — runs inside GitHub Actions."""
import json
import os
import re
import sys
...
def extract(text: str) -> dict:
    ...
    try:
        ...
        if not m:
            raise ValueError(f"AI did not return JSON. Raw (truncated): {text[:800]}")
...
def prompt(provider: str, c: dict) -> str:
...
def gemini(c: dict) -> dict:
    key = os.environ["GEMINI_API_KEY"]
    model = os.environ.get("GEMINI_MODEL", "gemini-2.5-flash")
    payload = {
        "contents": [{"role": "user", "parts": [{"text": prompt("Gemini", c)}]}],
        "generationConfig": {"responseMimeType": "application/json"},
    }
    url = f"https://generativelanguage.googleapis.com/v1beta/models/{model}:generateContent"
    d = _http_json(url, payload, {"Content-Type": "application/json", "x-goog-api-key": key})
    try:
        text = d["candidates"][0]["content"]["parts"][0]["text"]
    except (KeyError, IndexError, TypeError) as e:
        raise RuntimeError(f"Unexpected Gemini response shape: {json.dumps(d)[:800]}") from e
    return extract(text)


def chatgpt(c: dict) -> dict:
    key = os.environ["CHATGPT_API_KEY"]
    model = os.environ.get("CHATGPT_MODEL", "gpt-5.6")
    payload = {
        "model": model,
        "input": [{"role": "user", "content": prompt("ChatGPT", c)}],
    }
    d = _http_json(
    ...
    # Responses API: prefer output_text convenience field, then walk output items.
    text = d.get("output_text") or ""
    if not text:
        chunks = []
        for item in d.get("output") or []:
            for x in item.get("content") or []:
                if x.get("type") in ("output_text", "text") and x.get("text"):
                    chunks.append(x["text"])
        text = "\n".join(chunks)
    if not text:
        raise RuntimeError(f"Empty ChatGPT response. Body (truncated): {json.dumps(d)[:800]}")
    return extract(text)


def main() -> None:
    if len(sys.argv) != 3:
        raise SystemExit("usage: ai-room-agent.py gemini|chatgpt path")
    provider, path = sys.argv[1], Path(sys.argv[2])
    c = json.loads(path.read_text(encoding="utf-8"))
    expected = {
        "gemini": {"awaiting_gemini", "awaiting_gemini_counter"},
        "chatgpt": {"awaiting_chatgpt"},
    }[provider]
    if c.get("status") not in expected:
        print(f"skip: status={c.get('status')!r} not in {expected}")
        return
    ...
    try:
        ...
        if not reply:
            raise ValueError("AI returned empty reply")

        c["round"] = int(c.get("round", 0)) + 1
        c["updated_at"] = now()
        c["messages"].append(
            {
                "id": f"{provider}-{c['round']}-{int(datetime.now().timestamp())}",
                "speaker": provider,
                "text": reply,
                "created_at": now(),
                "round": c["round"],
        ...
        if was_agreed and agreed and c["round"] >= 2:
            c["status"] = "final"
            c["final_output"] = final or reply
            c["finalized_at"] = now()
        elif c["round"] >= MAX_ROUNDS:
            c["status"] = "final"
            c["final_output"] = final or reply
            c["finalized_at"] = now()
            c["max_rounds_reached"] = True
        elif provider == "gemini":
            c["status"] = "awaiting_chatgpt"
        else:
            c["status"] = "awaiting_gemini_counter"

        path.write_text(json.dumps(c, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
        print(f"ok: {provider} round={c['round']} status={c['status']} agreed={agreed}")
